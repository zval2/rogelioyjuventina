...
function updateCountdown(){
  ...
  if(now>=weddingStart&&now<weddingEnd){
    countdown.hidden=true;dayMessage.hidden=false;dayMessage.textContent="¡Hoy es nuestro gran día!";return;
  }
  if(now>=weddingEnd){
    countdown.hidden=true;dayMessage.hidden=false;dayMessage.textContent="¡Gracias por acompañarnos!";return;
  }
  countdown.hidden=false;dayMessage.hidden=true;
...
function fadeInMusic(){
  music.volume=0;
  const target=.82,duration=2200,started=performance.now();
  function step(now){
...
openBtn.addEventListener("click",async()=>{
  ...
  try{await music.play();fadeInMusic();}catch(error){console.warn("No se pudo iniciar la música:",error);}
